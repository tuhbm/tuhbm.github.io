...
사이트는 **날짜가 된 글만 공개**한다(`_config.yml`의 `future: false`). GitHub Actions가 gh-pages 푸시 때와 **매일 09:05~14:05(KST) 매시**(`schedule` cron `5 0-5 * * *`)에 재빌드해, 그 사이 날짜가 된 글을 공개한다. PC가 꺼져 있어도 GitHub 서버에서 동작한다.
