...
5. **대표 이미지 제작 (필수)**: 글마다 대표 이미지 1장. **제미나이 등 외부 API를 쓰지 않고 클로드가 직접 SVG를 제작**한 뒤 `tools/svg2png`로 PNG 변환한다 (사용자 결정, 2026-08-29).
   ...
   - SVG 함정 추가(2026-09-21): **가로로 긴 선(stroke)에 세로 방향 그라데이션을 지정하면 렌더링이 통째로 사라진다**(objectBoundingBox 높이 0). 선에는 단색을 쓰고, 그라데이션은 면에만 쓴다.
...
## 공통: 예약 발행 (2026-09-17 도입 — 주말·연휴용)

사이트는 **날짜가 된 글만 공개**한다(`_config.yml`의 `future: false`). GitHub Actions가 gh-pages 푸시 때와 **매일 09:17~14:47(KST) 30분마다**(`schedule` cron `17,47 0-5 * * *`, 2026-09-18 정각 슬롯에서 변경 — 첫날 14:05 예약 실행이 누락됨)에 재빌드해, 그 사이 날짜가 된 글을 공개한다. PC가 꺼져 있어도 GitHub 서버에서 동작한다.

- **운용 방식(사용자 확정)**: 평일은 지금처럼 당일 작성·발행. **주말과 연휴 글은 직전 평일에 미리 써서 푸시**한다(예: 금요일에 토·일 20편). 사용자가 요청할 때 진행한다.
- **예약 글 작성 규칙**: `date`를 발행일 09:00~13:30(30분 간격)으로 지정. 본문·description에 "오늘/어제/내일" 같은 상대 표현 대신 절대 날짜를 쓴다. 발행일까지 1~3일이 지나도 유효한 주제(제도 시행 안내, 연휴 교통·생활 정보, 과학·해외 토픽, 트렌드, 일정이 확정된 행사·경기 예고)를 고르고, 결과가 곧 나오는 속보(경기 결과 대기, 협상 타결 여부 등)는 피한다. 미리 쓰는 여러 날짜분끼리도 주제가 겹치지 않게 한다.
- **로컬 검증**: 일반 `npx hexo generate`는 미래 날짜 글을 빼고 빌드한다(평일 오전에 돌리면 당일 늦은 시각 글도 빠짐). 예약 글까지 포함해 검증하려면 `npx hexo clean && node tools/preview-future.js`로 빌드한 뒤 `public/<날짜>/` 편수·볼드 아티팩트·태그 링크를 확인한다.
- **배포 후 확인**: 푸시 직후 라이브 URL 검증은 **시각이 이미 지난 글**로만 한다. 예약 글은 해당일 예약 빌드 이후에 공개된다(cron은 GitHub 부하로 수 분~수십 분 늦을 수 있음). 수동 재빌드는 GitHub Actions 탭의 workflow_dispatch.
- **시간대 주의**: 빌드 단계에 `TZ: Asia/Seoul`을 준다. `_config.yml`의 `timezone`은 **비워둘 것** — `Asia/Seoul`로 두면 UTC 러너에서 00~09시에 작성된 옛 글 9편(cssArchitecture, highBuff_260225 등)의 퍼머링크 날짜가 하루 앞당겨져 기존 URL이 깨진다(2026-09-17 검증).
- 예약 빌드는 파일 수정시각 기반 값(atom.xml 등)이 바뀌어 새 글이 없어도 master에 `deploy:` 커밋이 생길 수 있다. 정상이다.
...
- **2026-09-17 (Windows PC)**: **예약 발행 도입**(사용자 요청 — PC가 꺼진 주말·연휴에도 발행). ①`_config.yml` `future: true`→`false` ②`hexo.yml`에 `schedule`(매일 09:05~14:05 KST 매시)·`workflow_dispatch`·`concurrency` 추가 ③빌드 단계 `TZ: Asia/Seoul`(timezone 설정은 옛 글 URL을 깨뜨려 사용 안 함) ④예약 글 포함 로컬 검증용 `tools/preview-future.js` 추가. 평일은 기존대로 당일 작성, 주말·연휴분은 직전 평일에 미리 작성. 세부 규칙은 위 "공통: 예약 발행" 참조. 변경 전후 빌드의 URL 목록을 비교해 기존 글 주소가 바뀌지 않음을 확인했다(차이는 아직 시각이 안 된 당일 글뿐).
- **2026-09-15 (Windows PC)**: **주제 선정 기준에 "흥미도 우선" 추가**(사용자 요청 — 조회수 부진 대응). 파급력만이 아니라 독자가 클릭하고 싶어질 주제를 먼저 고른다: ①다음 기사 페이지 우측 "실시간 트렌드" 키워드와 겹치는 이슈 ②의외성 있는 수치 ③사람 이야기(미담·사연) ④생활에 바로 닿는 새 제도의 첫날 반응 ⑤신기한 과학·해외 토픽 ⑥팬덤이 큰 연예 소식 ⑦세대 트렌드(스몰엔딩·전업자녀 등). 딱딱한 정책·거시지표·기관 발표 기사는 하루 1~2편 이내로 줄이고, 분야 배분은 ±1을 넘겨 조정해도 된다. 사실 확인·저작권·법적 리스크 원칙은 그대로이며 자극적 사생활·논란성 가십은 계속 제외한다. 효과는 GA4 조회수로 확인할 것. daily-blog 스킬에도 반영했지만 스킬 파일은 PC 로컬에만 있으므로 맥북에서도 이 항목을 따를 것.
